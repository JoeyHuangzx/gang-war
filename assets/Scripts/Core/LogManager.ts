...
class LogManager {
  ...
  public static info(message: string | number, ...optionalParams: any[]) {
    if (this.currentLevel <= LogLevel.INFO) {
      console.log('%c[INFO] %s', 'color:rgb(159, 246, 9);', message, ...optionalParams);
    }
  }
  // 输出 WARN 级别的日志
  public static warn(message: string | number, ...optionalParams: any[]) {
    if (this.currentLevel <= LogLevel.WARN) {
      console.log('%c[WARN] %s', 'color:rgb(205, 92, 92);', message, ...optionalParams);
    }
  }
  // 输出 ERROR 级别的日志
  public static error(message: string, ...optionalParams: any[]) {
    if (this.currentLevel <= LogLevel.ERROR) {
      console.log('%c[ERROR] %s', 'color:rgb(255, 0, 0);', message, ...optionalParams);
    }
  }
}
